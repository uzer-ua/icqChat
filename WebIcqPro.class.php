...
class WebIcqPro_SNAC extends WebIcqPro_TLV {
	...
	0x0B => 'ClientIBCMAck',
	0x0C => 'ServerIBCMAck',
	0x14 => 'MiniTypingNotification',
	'version' => 0x01
	),
	0x06 => array( // Invitation
	'version' => 0x01,
	),
...
//	'563FC8090B6F41BD9F79422609DFA2F3',
	...
	'563fc8090b6f41bd9f79422609dfa2f3',
	'094613494c7f11d18222444553540000',
	'0946134c4c7f11d18222444553540000',
	'0946134d4c7f11d18222444553540000',
	'178c2d9bdaa545bb8ddbf3bdbd53a10a',
	'0946134e4c7f11d18222444553540000',
	'97b12751243c4334ad22d6abf73f1492',
	'094600004c7f11d18222444553540000',
	'67361515612d4c078f3dbde6408ea041',
	'094613434c7f11d18222444553540000',
	'094613444c7f11d18222444553540000',
	'b99708b53a924202b069f1e757bb2e17',
	'1a093c6cd7fd4ec59d51a6474e34f5a0',
...
//	'97B12751243C4334AD22D6ABF73F1492', //  Client supports RTF messages. This capability currently used by ICQ service and ICQ clients.
	...
	'0946134c4c7f11d18222444553540000',
	'563fc8090b6f41bd9f79422609dfa2f3',
	'563fc8090b6f41514950203230303561',
	'd3d453198b32403bacc7d1a9e2b5813e',
	'1a093c6cd7fd4ec59d51a6474e34f5a0',
	);
	...
	protected $x_status = array('status'=>'','msg'=>'','extmsg'=>'');
	...
	protected function ClientStatus($args)
	{
		...
		{
			$snac = pack('nnnN',0x01,0x1E,0x00,0x1E);
			if (strlen($text)>251)
			{
				$snac .= $this->packTLV(0x1D, pack('nnnn', 0x02,0x00,0x0E,0x00));
			}
			else
			{
				$mood = 'icqmood'.$this->x_moods[$status];
				$snac .= $this->packTLV(0x1D, pack('nccna*nnna*', 0x02,0x04,(strlen($text)+4),strlen($text),$text,0x00,0x0E,strlen($mood),$mood));
			}
		}
		return $snac;
	}

	protected function ServerStatus($data)
	{
		return true;
	}
	...
	protected function ClientLocationInfo($args)
	{
		...
		return pack('nnnN',0x02,0x04,0x00,0x00).$this->packTLV(0x05, implode($this->capabilities).$this->user_agent_capability[$this->agent], 'H*');
	}
	protected function ClientLocationInfo2($args)
	{
		return pack('nnnNnn',0x02,0x04,0x00,0x04,0x04,0x00);
	}
	...
	protected function ClientSetIBCMParams($args)
	{
		...
		return $this->__header(0x04, 0x02).pack('nNnnnN', $channel, 0x0b, 0x1f40, 0x03e7, 0X03e7, 0x00);
	}
	...
	/**
	 * Create/parse Mini Typing Notification
	 * @param mixed
	 * @return array
	 */
	protected function MiniTypingNotification($args)
	{
		if (is_array($args))	//pack notification
		{
			extract($args);
			$uin_size = strlen($uin);
			$cookie = 0x00000000;
			$snack = $this->__header(0x04, 0x14).pack('dnca*n', $cookie, 0x0001, $uin_size, $uin, $type);
			$return = array('return' => $cookie, 'data' => $snack);
		}
		else	//unpack notification
		{
			$msg = unpack('dcookie/nchannel/clen',$args);
			$data = substr($args,11);
			$return = array('type'=>'mtn','cookie'=>$msg['cookie'],'channel'=>$msg['channel']);
			$return['from'] = substr($data,0,$msg['len']);
			$data = substr($data,$msg['len']);
			$msg = unpack('nnotify',$data);
			$return['notify'] = $msg['notify'];
			unset($msg);
		}
		return $return;
	}
					...
					case 2:
						if ($return['channel'] == 1)
						{
							...
							if (isset($subtlvs[0x0101]))
							{
								...
								//echo 'Encoding';dd($return['encoding']);
								...
								//echo $return['message']."\n";echo strtoupper(bin2hex($return['message']))."\n-------\n";
...
//			$this->writeFlap('ClientIBCMAck', $return);
	...
	private function parseServerGreeting($return, $meta, $data)
	{
		...
		$return['plugin'] = array();
		$return['plugin']['capability'] = bin2hex(substr($data, 3, 16));
		switch ($return['plugin']['capability'])
		{
			case $this->message_capabilities['PLUGIN']['XTRAZ_SCRIPT']:
			{
				$data = substr($data,20);
				$meta = unpack('nlength',$data);
				$data = substr($data,5);
				$return['plugin']['header'] = substr($data,0,$meta['length']);
				$data = substr($data,19+$meta['length']);
				$meta = unpack('vlength/nfake',$data);	//why little endian?
				//$return['plugin']['description'] = substr($data,4,$meta['length']);
				//$return['plugin']['description'] = str_replace(array('&lt;','&gt;'),array('<','>'),$return['description']);
				//$parser = new xmlparser($return['description']);
				//$return['plugin']['description'] = $parser->parse();
				
				$return['description'] = 'parse needed';
				
				//And finally I'm tired. Later
				$this->writeFlap('ClientIBCMAck',$return);
			}break;
			default:{
				$return['debugmsg'] = 'Unknown/unsupported plugin';
				$this->log('SG', $data, true);
				return $return;
			}
		}
	...
	protected function ClientIBCMAck($args)
	{
		if (is_array($args))
		{
			...
			{
				case 0x02:
					if (isset($plugin))		//straz response
					{
						$snack .= pack('nccH*nNcnnnH*H*H*ca*cnH*NNa*',0x1B00,$this->protocol_version,0x00,'00000000000000000000000000000000',0x00,0x01000000,0x00,0xc30e,0x0e00,0xc30e,'000000000000000000000000','1A00000001000100004F003B60B3EFD8','2A6C45A4E09C5A5E67E86508002A0000',0x00,'Script Plug-in: Remote Notification Arrive',0x00,0x01,'000000000000000000000000',0xfa010000,0xf6010000,"<NR><RES>&lt;ret event='OnRemoteNotification'&gt;&lt;srv&gt;&lt;id&gt;cAwaySrv&lt;/id&gt;&lt;val srv_id='cAwaySrv'&gt;&lt;Root&gt;&lt;CASXtraSetAwayMessage&gt;&lt;/CASXtraSetAwayMessage&gt;&lt;uin&gt;".$from."&lt;/uin&gt;&lt;index&gt;&lt;title&gt;".$this->x_status['msg']."&lt;/title&gt;&lt;desc&gt;".$this->x_status['extmsg']."&lt;/desc&gt;&lt;/Root&gt;\r\n&lt;/val&gt;&lt;/srv&gt;&lt;srv&gt;&lt;id&gt;cRandomizerSrv&lt;/id&gt;&lt;val srv_id='cRandomizerSrv'&gt;undefined&lt;/val&gt;&lt;/srv&gt;&lt;/ret&gt;</RES></NR>\r\n");
					}
					else
					{
	...
	protected function ServerSSI($data, $flag = 0)
	{
		...
		if ($flag == 0 || $flag == 32768) {
			...
			return $this->createResponse("contactlist",array($this->contact_list_groups,$this->contact_list));
		}
		return true;
	}
	...
	protected function ClientMd5Login($args)
	{
		...
		return $this->__header(0x17, 0x02).
				...
				$this->packTLV(0x18, 0x05, 'n').
				$this->packTLV(0x19, 0x00, 'n').
				$this->packTLV(0x1A, 0x68, 'n').
				$this->packTLV(0x14, 0x00000753, 'N').
				$this->packTLV(0x0F, 'en').
				$this->packTLV(0x0E, 'us');//.
	...
	protected function set_x_status($status, $msg, $extmsg){
		$this->x_status['status'] = $status;
		$this->x_status['msg'] = $msg;
		$this->x_status['extmsg'] = $extmsg;
	}
}
...
class WebIcqPro extends WebIcqPro_Socet {
					...
					if ($reconect)
					{
						...
						if ($this->socetOpen())
						{
							...
							if ($this->helloFlap($flap))
							{
								...
								$this->writeFlap('ClientRequestSelfInfo');
	 ...
	 * @return boolean
	 */
	public function setXStatus($status = '', $msg = '', $extmsg = '')
	{
		...
		if ($msg)
		{
			$this->writeFlap('ClientLocationInfo2');
			$this->writeFlap('ClientStatus',array('status' => $status, 'text'=>$msg.($extmsg?' '.$extmsg:'')));
			//TODO: parse acks
			$this->set_x_status($status, $msg, $extmsg);
		}
		return $return;
	}
	...
	{
		//$message = iconv('windows-1251','UTF-16',charset_x_win($message));
		//$message = mb_convert_encoding($message,'UTF-16');
		//$message = substr($message,2);
		//$message = charset_x_win($message);
		//$message = iconv('windows-1251','ASCII',$message);
		$uin = str_replace('-', '', $uin);
		$this->writeFlap('ClientIdleTime');
	...
	public function sendTypingNotification($uin,$type)
	{
		$uin = str_replace('-', '', $uin);
		return $this->writeFlap('MiniTypingNotification', array('uin' => $uin, 'type' => $type));
	}
}
?>